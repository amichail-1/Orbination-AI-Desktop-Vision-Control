...
// Enable per-monitor DPI awareness for correct coordinates on multi-monitor setups
[DllImport("user32.dll")]
static extern bool SetProcessDpiAwarenessContext(nint value);
SetProcessDpiAwarenessContext((nint)(-4)); // DPI_AWARENESS_CONTEXT_PER_MONITOR_AWARE_V2
...
builder.Services
    .AddMcpServer(options =>
    {
        ...
        options.ServerInstructions = """
            ## Desktop Control MCP - Tool Usage Guide
            ...
            1. **ocr_window** — Read ALL visible text from a window with click coordinates. Use this FIRST to understand what's on screen.
               Returns exact text strings you can use in click_element, run_sequence, etc.
            ...
            2. **click_menu_item** — Navigate parent > child menus in one call. Keeps submenus open with smooth mouse movement.
            3. **run_sequence** — Batch multiple hotkeys, waits, focus changes in ONE call. Use for keyboard workflows (Ctrl+A, Ctrl+V, etc).
               NOTE: For clicking buttons, prefer click_element over run_sequence's ocr_click — it has better matching.
            4. **set_clipboard + keyboard_hotkey (ctrl+v)** or **paste_text** — For large text (XML, code, JSON).
            5. **mouse_click x,y** — Direct coordinate click. ONLY when text-based tools fail. Get coordinates from ocr_window or get_window_details first.
            ...
            - Do NOT use screenshot_to_file to understand UI — use ocr_window or get_window_details instead. They give exact text and coordinates.
            - Do NOT screenshot after every action. Use ocr_window to check state when needed.
            - Do NOT guess button text — OCR the window first, then use the exact text from OCR results.
            - Do NOT use run_sequence ocr_click for buttons — use click_element which has better UIAutomation + OCR fallback.
            - Do NOT use keyboard_type for large text — use paste_text or set_clipboard + ctrl+v.
            - Do NOT use mouse_click with guessed coordinates — get coordinates from ocr_window or get_window_details first.
            """;
    })
